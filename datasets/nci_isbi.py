"""NCI-ISBI prostate image and dual-pseudo-mask datasets."""
...
class NCIISBIDataset(BaseImageDataset):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.images = [
            self.root / "images" / f"{name}.dcm" for name in self.sample_list
        ]
    ...
    def _fetch_data(self, idx):
        return {
            "image": pydicom.dcmread(self.images[idx]).pixel_array.astype(np.float32),
            "label": np.asarray(
...
class NCIISBIGazeDataset(NCIISBIDataset):
