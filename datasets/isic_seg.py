"""ISIC image and dual-pseudo-mask datasets."""

from pathlib import Path

import numpy as np
from monai.transforms import Compose, EnsureChannelFirstd, RandFlipd, Resized
...
class ISICDataset(BaseImageDataset):
    def __init__(self, *args, **kwargs):
        ...
        self.images = [
            self.root / "images" / f"{name}.jpg" for name in self.sample_list
        ]
    ...
    def _fetch_data(self, idx):
        return {
            "image": np.asarray(
                Image.open(self.images[idx]).convert("RGB"), dtype=np.float32
            ),
            "label": np.asarray(
    ...
    def _transform_custom(self, data):
        label = data["label"].float() / 255.0
        data["label"] = (label > 0.5).long()
        return data


class ISICGazeDataset(ISICDataset):
    ...
    def __init__(self, pseudo_high_root, pseudo_low_root, *args, **kwargs):
        ...
        super().__init__(*args, **kwargs)
        self.labelshigh = [
            Path(pseudo_high_root) / f"{name}.jpg" for name in self.sample_list
        ]
        self.labelslow = [
            Path(pseudo_low_root) / f"{name}.jpg" for name in self.sample_list
        ]
    ...
    def get_transform(self):
        ...
        transforms = [
            ...
            EnsureChannelFirstd(keys=["label", *pseudo_keys], channel_dim="no_channel"),
    ...
    def _fetch_data(self, idx):
        ...
        data["pseudo_label_1"] = (
            np.asarray(Image.open(self.labelshigh[idx]).convert("L"), dtype=np.float32)
            / 255.0
        )
        data["pseudo_label_2"] = (
            np.asarray(Image.open(self.labelslow[idx]).convert("L"), dtype=np.float32)
            / 255.0
        )
        return data
